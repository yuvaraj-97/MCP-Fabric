...
import { createWorkloadRegistry } from "../../gateway/workload-registry/workload-registry.js";
import { resolveWorkloadAffinity } from "../../gateway/workload-registry/workload-affinity-resolver.js";
...
export function createHttpSseGatewayController({
  ...
  return {
    router,
    sessionRegistry: resolvedSessionRegistry,
    workloadRegistry: resolvedWorkloadRegistry,
    applications,
    observer,
    describeRegistry() {
      return {
    ...
    async handleGatewayMessage(body, headers = {}) {
      ...
      const workloadAffinity = resolveWorkloadAffinity(body, {
        stateHandleKeys: resolvedOperatorConfig.stateHandleKeys,
        findToolSchema,
      });

      if (isModern) {
        if (workloadAffinity) {
          await resolvedWorkloadRegistry.pruneExpired?.();
          let record = await resolvedWorkloadRegistry.get(workloadAffinity.id);
          let serverInstanceId;
          let routingReason;
          let placementCreated = false;
          let placementReassigned = false;

          const healthyInstances = router.listInstances().filter((i) => i.healthy && i.acceptingNewSessions);

          if (record) {
            const assignedInstance = router.listInstances().find(
              (i) => i.serverInstanceId === record.serverInstanceId
            );
            if (assignedInstance?.healthy) {
              serverInstanceId = record.serverInstanceId;
              routingReason = "workload-affinity-sticky";
              await resolvedWorkloadRegistry.touch(workloadAffinity.id, effectiveSessionTtlMs);
            } else {
              ...
              serverInstanceId = healthyInstances[0].serverInstanceId;
              routingReason = "workload-recovery-reassignment";
              await resolvedWorkloadRegistry.reassign(workloadAffinity.id, serverInstanceId);
              placementReassigned = true;
            }
          } else {
            if (healthyInstances.length === 0) {
              const error = new Error("No healthy server instance is accepting new sessions");
            ...
            healthyInstances.sort((a, b) => a.load - b.load);
            serverInstanceId = healthyInstances[0].serverInstanceId;
            routingReason = "workload-placement-creation";
            record = await resolvedWorkloadRegistry.create(workloadAffinity.id, {
              kind: workloadAffinity.kind,
              serverInstanceId,
              expiresAt: now() + effectiveSessionTtlMs,
              metadata: {
                runtimeMode: "sticky",
              },
            });
          ...
          observer.record("workload.route.selected", {
            method,
            mcpMethod: mcpMethodHeader || method,
            mcpName: mcpNameHeader || body.params?.name || null,
          ...
          observer.record("stateless.route.selected", {
            method,
            mcpMethod: mcpMethodHeader || method,
            mcpName: mcpNameHeader || null,
            serverInstanceId: selectedInstance.serverInstanceId,
            routingReason: "stateless-load-balanced",
            traceContext,
          });
      ...
      const stateHandle = workloadAffinity ? workloadAffinity.id : (body.sessionId ?? (method === "initialize" ? randomUUID() : undefined));
      const sessionId = stateHandle;
      if (stateHandle) {
...
function createGatewayContext({ sessionId, route, reusedExistingSession, eventStreams, traceContext }) {
  return {
    sessionId,
    transport: "http-sse",
    metadata: {
