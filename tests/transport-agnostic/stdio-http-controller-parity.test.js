...
test("stdio and HTTP/SSE controller expose the same initialize and echo behavior without sockets", async (t) => {
  ...
  const stdioInitialize = await sendStdioMessage({
    id: 1,
    method: "initialize",
  ...
  const stdioEcho = await sendStdioMessage({
    id: 2,
    method: "echo",
  ...
  const httpInitialize = await controller.handleGatewayMessage({
    method: "initialize",
    params: { clientId: "parity-client" },
  });

  const httpEcho = await controller.handleGatewayMessage({
    method: "echo",
...
async function sendStdioMessage(payload) {
