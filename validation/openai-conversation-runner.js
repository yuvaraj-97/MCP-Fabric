...
export class OpenAIConversationRunner {
  #baseRunner;
  #client;
  #scenarioId;
  #title;
  ...
  constructor({ scenarioId, title, baseRunner, client = null }) {
  ...
  describe() {
    ...
    return {
      ...
      title: this.#title,
      audience:
        "Uses the OpenAI API to generate assistant replies while the real validation actions still run through the shared transport-backed validation flow.",
      rootDir: baseState.rootDir,
      createdFile: baseState.createdFile,
      provider: {
  ...
  getState() {
    ...
    return {
      ...description,
      observability: this.#baseRunner.getState().observability,
  ...
  async runStep(stepId) {
    ...
    await this.runStepStream(stepId, (event) => {
  ...
  async runStepStream(stepId, emit) {
    ...
    if (typeof client.createResponseStream === "function") {
      ...
      finalResponseId = secondResponse.id ?? null;
      assistantSummary = extractAssistantText(secondResponse);
      emit({
        type: "openai.response.created",
        phase: "assistant-summary",
        responseId: finalResponseId,
